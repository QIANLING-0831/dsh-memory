...
const CJK_QUERY_SQLITE_SCHEMA_VERSION = 1;
/** SQLite application id protecting unrelated databases from derived resets. */
const CJK_QUERY_SQLITE_APPLICATION_ID = 1146308690;
const DERIVED_USER_TABLES = new Set([
	...
	"persisted_docs_cjk",
	"persisted_docs_cjk_data",
	"persisted_docs_cjk_idx",
	"persisted_docs_cjk_content",
	"persisted_docs_cjk_docsize",
	"persisted_docs_cjk_config"
]);
/**
...
function ensurePersistentSchema(db) {
	db.exec(`PRAGMA application_id = ${CJK_QUERY_SQLITE_APPLICATION_ID}`);
	db.exec(`
  ...
  `);
	db.exec(`
    CREATE VIRTUAL TABLE IF NOT EXISTS persisted_docs_cjk USING fts5(
      text,
      session_id UNINDEXED,
      seq UNINDEXED,
      type UNINDEXED,
  ...
  `);
	db.exec(`PRAGMA user_version = ${CJK_QUERY_SQLITE_SCHEMA_VERSION}`);
}
function ensureTemporarySchema(db) {
  ...
  `);
	db.exec(`
    CREATE VIRTUAL TABLE IF NOT EXISTS temp.live_docs_cjk USING fts5(
      text,
      session_id UNINDEXED,
      seq UNINDEXED,
      type UNINDEXED,
...
const CJK_QUERY_SQLITE_PATH_KEY = "launcherSessionQueryPath";
/** Default result page size. */
const CJK_QUERY_SQLITE_DEFAULT_LIMIT = 20;
/** Maximum accepted result page size. */
const CJK_QUERY_SQLITE_MAX_LIMIT = 100;
/** Default maximum snippet length in Unicode code points. */
const CJK_QUERY_SQLITE_SNIPPET_CHARS = 240;
 ...
 /* v8 ignore next 5 -- ROLLBACK failure requires a SQLite double fault; the original failure remains actionable. */
	...
	_deleteSession(source, id) {
		...
		if (source === "persisted") {
			...
			db.prepare("DELETE FROM persisted_docs_cjk WHERE session_id = ?").run(id);
			db.prepare("DELETE FROM persisted_sessions WHERE id = ?").run(id);
		} else {
			db.prepare("DELETE FROM temp.live_docs WHERE session_id = ?").run(id);
			db.prepare("DELETE FROM temp.live_docs_cjk WHERE session_id = ?").run(id);
    ...
    `).run(...headerBindings(entry.header), revision, generation);
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
		const insertCjk = db.prepare(`
      INSERT INTO persisted_docs_cjk (text, session_id, seq, type, time, surface, codepoint_length)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
		for (const document of entry.documents) {
			...
			insertCjk.run(text, document.sessionId, document.seq, document.type, document.time, document.surface, Array.from(text).length);
		}
	}
    ...
    `);
		const insertCjk = db.prepare(`
      INSERT INTO temp.live_docs_cjk (text, session_id, seq, type, time, surface, codepoint_length)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `);
		for (const document of entry.documents) {
			const text = sanitizeFtsText(document.text);
...
function selectedDocumentsSql(useCjk) {
	...
	return { sql: `WITH candidates AS (
      SELECT
        ...
        highlight(${persistedDocs}, 0, ?, ?) AS marked_text,
      ...
      SELECT
        ...
        highlight(${liveDocs}, 0, ?, ?) AS marked_text,
...
//#endregion
export { CJK_QUERY_SQLITE_APPLICATION_ID, CJK_QUERY_SQLITE_DEFAULT_LIMIT, CJK_QUERY_SQLITE_MAX_LIMIT, CJK_QUERY_SQLITE_PATH_KEY, CJK_QUERY_SQLITE_SCHEMA_VERSION, CJK_QUERY_SQLITE_SNIPPET_CHARS, CjkSessionQueryEngine, CjkSessionQueryEngine as default };
