...
**一句话定位**：唯一在修 DSH 记忆地基（CJK 全文检索）+ 唯一做 Token 去重 + 唯一有 KV-safe 注入论证的**记忆全家桶**——不是"又一个记忆插件"。
...
2. **Token 去重**——`dsh-tool-result-dedup` 从输入侧省 Token，与记忆检索正交，生态少见。
3. **KV-safe 稳定注入**——基于源码级验证（`buildRequest` deepFreeze / KV 前缀缓存失效 / 持久化日志污染）得出的注入纪律；多数"每步自动注入"型插件会踩同一道墙（如 dsh-layered-memory）。
4. **compaction 来源定位**——近无损压缩 + 每条摘要可溯源到 spill 文件/seq 区间，记忆可审计。

### 验证与测试

- **55 单测**（6 包，node --test）：CJK 检索 12、混合检索 8、去重 8、core 8、tool 8、compaction 7；
...
| **Skills 自进化 / 学习循环** | 🔴 **dsh-memory-evolve（205⭐）已完整实现**（技能自我进化 + 管理器） |

**结论**：Hermes 式自进化不是空白（此前判断已修正）。**本仓库的差异化在地基**——CJK 检索（生态唯一）+ Token 去重 + KV-safe 注入 + compaction 定位；自进化方向可与 dsh-memory-evolve 互补而非竞争。
